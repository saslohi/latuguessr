...
// Ninja: https://tiles.hel.ninja/styles/hel-osm-bright/{z}/{x}/{y}@2x@fi.png
// Prod: https://maptiles.api.hel.fi/styles/hel-osm-bright-fi/{z}/{x}/{y}@2x.png

var hkiOsm = L.tileLayer('https://maptiles.api.hel.fi/styles/hel-osm-bright-fi/{z}/{x}/{y}@2x.png', {
    maxZoom: 18,
...
var geojsonMarkerOptionsArvaus = {
  radius: 8,
  fillColor: "#5933ffff",
  color: "#0000",
  weight: 1,
...
function piilotaArvaukset() {
  mymap.removeLayer(arvausLayer)
}
...
mymap.on('click', function(ev) {
    ...
    arvaus.latitude = ev.latlng.lat
    arvaus.longitude = ev.latlng.lng
});
...
let ohjeet = `<button id="tarkastaArvausNappi">Tarkasta arvaus</button>`
  ...
  if (aktiivinenTehtava != ladut.length) {
  ohjeet = `
